...
def get_net(input_shape):
    ...
    output = FinalModel(inputs, tail_prev, tail)

    return inputs, xception.input, output


def bise_net(input_shape, num_classes=1):
    inputs, xception_inputs, ans = get_net(input_shape)

    if num_classes == 1:
        output = Conv2D(num_classes, (1, 1), strides=(1, 1), padding='same', name='context', activation='sigmoid')(ans)
    else:
